...
serve(async (req: Request): Promise<Response> => {
  ...
  if (event === 'payment.captured') {
    ...
    if (!paymentEntity?.order_id || !paymentEntity?.id) {
      console.error('razorpay-webhook: payment.captured missing order_id or payment id', paymentEntity);
    ...
    if (dbError) {
      console.error(
        `razorpay-webhook: DB update failed for order ${razorpayOrderId}`,
        dbError,
      );
