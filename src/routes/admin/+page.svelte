...
<div class="p-8">
	...
	<div class="mt-8 grid grid-cols-1 gap-5 sm:grid-cols-3">
		<div class="shadow-ambient rounded-3xl bg-surface-card p-6">
			...
			<p class="mt-2 font-display text-4xl font-semibold text-on-surface">
				{data.stats.totalOrders}
			</p>
		</div>
		<div class="shadow-ambient rounded-3xl bg-surface-card p-6">
			<p class="font-body text-sm text-on-surface-muted">Total Revenue</p>
			<p class="mt-2 font-display text-4xl font-semibold text-primary">
				...
				<ul class="space-y-3">
					{#each data.lowStock as product}
						<li class="flex items-center justify-between">
							...
							<span
								class="rounded-full px-3 py-0.5 font-body text-xs font-semibold {product.stock === 0
									? 'bg-primary/10 text-primary'
									: 'bg-mustard/40 text-tertiary'}"
							>
