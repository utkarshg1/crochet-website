		...
		{#each statuses as s}
			<a
				...
				class="chip px-4 py-1.5 font-body text-sm font-medium transition-colors
					...
					: 'shadow-ambient bg-surface-card text-on-surface-muted hover:bg-surface-low'}"
			>
				{statusLabels[s]}
			</a>
		{/each}
			...
			{#each data.orders as order}
				...
				<div class="shadow-ambient overflow-hidden rounded-3xl bg-surface-card">
					...
					<div class="flex items-center gap-4 p-5">
						<div class="min-w-0 flex-1">
							<div class="flex flex-wrap items-center gap-3">
								...
								<span
									class="chip px-2.5 py-0.5 font-body text-xs font-semibold {statusColors[
										order.status
									] ?? ''}"
								>
									{order.status}
						...
						<form method="POST" action="?/updateStatus" use:enhance class="flex-shrink-0">
							...
							<select
								...
								class="rounded-xl border border-on-surface/10 bg-surface-high px-3 py-1.5 font-body text-xs text-on-surface focus:border-primary/50 focus:outline-none"
							>
						...
						>
							<svg
								class="h-4 w-4 transition-transform {expandedOrder === order.id
									? 'rotate-180'
									: ''}"
								viewBox="0 0 20 20"
								fill="currentColor"
					...
					{#if expandedOrder === order.id}
						<div class="border-t border-surface-low bg-surface px-5 py-4">
							<div class="grid gap-4 sm:grid-cols-2">
								<div>
									<p
										class="mb-2 font-body text-xs font-semibold tracking-wider text-on-surface-muted uppercase"
									>
										Items
									</p>
										...
										{#each items as item}
											<li class="font-body text-sm text-on-surface">
												...
												<span class="text-on-surface-muted"
													>— {formatPrice(item.price_paise * item.qty)}</span
												>
											</li>
										{/each}
									</ul>
								...
								<div>
									...
									<address class="font-body text-sm leading-relaxed text-on-surface not-italic">
									...
									{#if order.razorpay_payment_id}
										<p class="mt-2 font-body text-xs text-on-surface-muted">
											Payment ID: <code class="font-mono text-secondary"
												>{order.razorpay_payment_id}</code
											>
										</p>
									{/if}
								</div>
